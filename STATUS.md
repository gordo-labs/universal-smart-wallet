...
- **Execution:** 28 atomic tasks with explicit dependencies, parallel waves,
  acceptance criteria, validation, and agent prompts. `SSW-001`–`SSW-007`,
  `SSW-015`, and `SSW-016` are Done.
...
1. `SSW-005` — nonce, replay, and verification-result domain logic.
2. `SSW-008` — version-pinned SD-JWT VC adapter.
3. `SSW-017` — opt-in ERC-4337 testnet adapter.
