...
- **Execution:** 28 atomic tasks with explicit dependencies, parallel waves,
  acceptance criteria, validation, and agent prompts. `SSW-001`–`SSW-003` and
  `SSW-006` are Done.
...
2. `SSW-015` — smart-account selection and Foundry harness.
3. `SSW-007` — encrypted vault and IndexedDB adapter (depends on SSW-006).
